...
> **Espelho WhatsApp (construído 2026-07-08):** a instância Z-API é **multi-device** e NÃO fornece histórico por API — o espelho real é a CF **`zapi-inbox`** recebendo os webhooks da Z-API e gravando em **`public.whatsapp_mensagens`** (stream único de produção: a CF grava SEMPRE em `public`, mesmo a `-uat`; Engine PRD e preview leem de `public`). Confirmado 🧑‍💼: não havia webhooks configurados na instância. Ativação (🤖 via gcloud/curl pós-merge): (1) env vars na `zapi-inbox-uat` — `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `SUPABASE_SCHEMA=public`, `WEBHOOK_SECRET`; (2) configurar na Z-API o webhook "ao receber" com **notifySentByMe** apontando p/ `https://.../zapi-inbox-uat?token=<WEBHOOK_SECRET>`. Histórico existe a partir da ativação (retroativo é impossível).
