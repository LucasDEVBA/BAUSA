...
test('migration do re-envio autorizado (26/08): janela fechada + exclusões', () => {
  const migReenvio = fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'migrations',
      '20260826210000_reenvio_lote_reativacao.sql'), 'utf8');
  ...
  assert.ok(migReenvio.includes('meeting_scheduled IS NOT TRUE'),
    'quem marcou reunião (ex.: Matheus Locatelli) não pode ser re-armado');
});
