...
| `/ads` (+`/desempenho`, `/roi`, `/campanha/[id]`) | CEO | **Meta Ads Manager** (épico A1/A1.5/A2): Campanhas (cards c/ criativo 512px, filtros status/busca, ativas primeiro, clique→detalhe), Desempenho (presets 7d–12m + range custom via querystring, 7 gráficos), CAC/ROI (tabela por campanha, motor do cac-queries), detalhe (hero, KPIs vida, ROI 12m, conjuntos, anúncios, leads da campanha). **Escrita A2**: pausar/reativar + orçamento diário — CEO-only, confirmação explícita, audit_logs, token separado `META_ACCESS_TOKEN_MANAGE` (leitura usa `META_ACCESS_TOKEN`; envs em Vercel prod+preview). Libs `src/lib/meta-ads.ts` (leitura) e `meta-ads-escrita.ts`; actions `src/lib/actions/ads.ts`. Guard `tests/ads-escrita-invariants.test.js`. Gotcha: client Supabase criado na page e injetado (dentro de lib concorrente = fetch failed no dev) |
| `/fluxos` | CEO | **Fluxos** — o "ManyChat próprio". Builder de conversa: 11 gatilhos (comentário em post/Reels, novo seguidor, resposta/menção de Story, DM, palavra-chave no WhatsApp, link ref, manual, agendado) × 12 tipos de bloco (mensagem, pergunta, botões, condição, IA, delay, tag, **captura**, handoff, ação no CRM, fim). Métricas e funil por bloco derivados de `fluxo_eventos` (append-only) em `src/lib/fluxos-queries.ts`; sugestão/diagnóstico de fluxo por IA em `src/lib/actions/fluxos-ia.ts`. **Métrica-rei = captura** (conversa que virou contato no funil). Motor: CF `fluxo-engine`. ⚠️ **Duas travas de envio** — `fluxos.ativo` (nasce FALSE) **e** o gate `fluxos_escopo` (card na própria tela: Desligado/Só estes/Global, fail-closed). Canal Instagram fica visível mas **não envia** até o App Review — o aviso está em `CANAL_CATALOG` |
