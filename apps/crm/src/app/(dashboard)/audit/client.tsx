...
export function AuditClient({ logs, tabelas }: AuditClientProps) {
  ...
  const selectClass =
    "appearance-none rounded-lg border border-input bg-card py-2 px-3 text-sm text-foreground transition-colors focus-visible:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/25";
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        eyebrow="Compliance"
        title="Audit Trail"
        description={`Histórico imutável de alterações — ${filtered.length} registros`}
        actions={
          <ExportCSVButton
      ...
      {/* Filtros */}
      <Card variant="plain" padding="md" className="flex flex-wrap items-center gap-3">
      ...
      {filtered.length === 0 ? (
        <Card variant="plain" padding="none" className="overflow-hidden">
          <EmptyState
            ...
            description="Ajuste os filtros para visualizar audit logs."
          />
        </Card>
      ) : (
        <Card variant="plain" padding="none" className="overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/50">
                  <th className="py-2.5 pl-4 pr-3 text-left text-eyebrow text-muted-foreground">
                    Data
                  </th>
                  <th className="px-3 py-2.5 text-left text-eyebrow text-muted-foreground">
                    Tabela
                  </th>
                  <th className="px-3 py-2.5 text-left text-eyebrow text-muted-foreground">
                    Operacao
